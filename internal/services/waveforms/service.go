...
func (s *service) GetWaveform(ctx context.Context, episodeID uint) (*models.Waveform, error) {
	...
	log.Printf("[DEBUG] Getting waveform for Podcast Index Episode ID: %d", episodeID)

	waveform, err := s.repo.GetByEpisodeID(ctx, episodeID)
	if err != nil {
		log.Printf("[DEBUG] Failed to get waveform for Podcast Index Episode ID %d: %v", episodeID, err)
		return nil, err
	}
...
func (s *service) SaveWaveform(ctx context.Context, waveform *models.Waveform) error {
	...
	if exists {
		log.Printf("[DEBUG] Updating existing waveform for Podcast Index Episode ID: %d", waveform.EpisodeID)
		return s.repo.Update(ctx, waveform)
	}

	log.Printf("[DEBUG] Creating new waveform for Podcast Index Episode ID: %d", waveform.EpisodeID)
...
func (s *service) DeleteWaveform(ctx context.Context, episodeID uint) error {
	...
	log.Printf("[DEBUG] Deleting waveform for Podcast Index Episode ID: %d", episodeID)
...
func (s *service) WaveformExists(ctx context.Context, episodeID uint) (bool, error) {
	...
	log.Printf("[DEBUG] Checking if waveform exists for Podcast Index Episode ID %d: %t", episodeID, exists)
	return exists, err
}
